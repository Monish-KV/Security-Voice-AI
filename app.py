...
# Optional high-risk notifications. Credentials stay outside the project files.
ALERT_EMAIL_TO = os.getenv("VOICESHIELD_ALERT_EMAIL_TO", "").strip()
ALERT_SMTP_HOST = os.getenv("VOICESHIELD_SMTP_HOST", "").strip()
ALERT_SMTP_PORT = int(os.getenv("VOICESHIELD_SMTP_PORT", "587"))
ALERT_SMTP_USER = os.getenv("VOICESHIELD_SMTP_USER", "").strip()
ALERT_SMTP_PASSWORD = os.getenv("VOICESHIELD_SMTP_PASSWORD", "")
ALERT_SMTP_FROM = os.getenv("VOICESHIELD_SMTP_FROM", ALERT_SMTP_USER).strip()
ALERT_TWILIO_SID = os.getenv("VOICESHIELD_TWILIO_ACCOUNT_SID", "").strip()
ALERT_TWILIO_TOKEN = os.getenv("VOICESHIELD_TWILIO_AUTH_TOKEN", "")
ALERT_TWILIO_FROM = os.getenv("VOICESHIELD_TWILIO_FROM", "").strip()
ALERT_TWILIO_TO = os.getenv("VOICESHIELD_TWILIO_TO", "").strip()
...
def build_classification_explanation(model_score: float, v2_score: float, v4_score: float) -> dict[str, str]:
    label = "FAKE" if model_score >= 50 else "REAL"
    if label == "FAKE":
        reason = (f"The ensemble classified this recording as likely AI-generated because the V2 "
                  f"deepfake score was {v2_score:.1f}% and the V4 deepfake score was {v4_score:.1f}%, "
                  f"giving a combined score of {model_score:.1f}%." )
    else:
        reason = (f"The ensemble classified this recording as likely genuine because the V2 "
                  f"deepfake score was {v2_score:.1f}% and the V4 deepfake score was {v4_score:.1f}%, "
                  f"giving a combined score of {model_score:.1f}%." )
    return {"label": label, "reason": reason}


def send_high_risk_notifications(result: dict[str, Any]) -> dict[str, Any]:
    """Send optional email/SMS alerts for HIGH risk without breaking analysis."""
    security = result["security"]
    if security["risk_level"] != "HIGH":
        return {"triggered": False, "email": "NOT_REQUIRED", "sms": "NOT_REQUIRED",
                "message": "Notifications are sent only for HIGH risk."}

    subject = f"VoiceShield HIGH RISK alert · {result['analysis_id']}"
    body = (f"VoiceShield AI detected a HIGH risk voice interaction.\n\n"
            f"Classification: {result['classification']}\n"
            f"Model deepfake score: {result['model_score']:.1f}%\n"
            f"Security risk score: {security['security_risk_score']:.1f}%\n"
            f"Recommended action: {security['recommended_action']}\n"
            f"Reason: {result['classification_reason']}\n"
            f"Security factors: {', '.join(security['reasons'])}\n"
            f"Analysis ID: {result['analysis_id']}\n\n"
            f"Recommended response: {security['recommendation']}")
    status = {"triggered": True, "email": "NOT_CONFIGURED", "sms": "NOT_CONFIGURED",
              "message": "HIGH risk detected."}

    if ALERT_SMTP_HOST and ALERT_EMAIL_TO and ALERT_SMTP_FROM:
        try:
            msg = EmailMessage()
            msg["Subject"] = subject
            msg["From"] = ALERT_SMTP_FROM
            ...
            app.logger.exception("High-risk email notification failed")
            status["email"] = f"FAILED: {type(exc).__name__}"

    if ALERT_TWILIO_SID and ALERT_TWILIO_TOKEN and ALERT_TWILIO_FROM and ALERT_TWILIO_TO:
        try:
            endpoint = f"https://api.twilio.com/2010-04-01/Accounts/{ALERT_TWILIO_SID}/Messages.json"
            payload = urlencode({"From": ALERT_TWILIO_FROM, "To": ALERT_TWILIO_TO, "Body": body[:1500]}).encode()
            req = Request(endpoint, data=payload, method="POST")
            import base64
            auth = base64.b64encode(f"{ALERT_TWILIO_SID}:{ALERT_TWILIO_TOKEN}".encode()).decode()
            req.add_header("Authorization", f"Basic {auth}")
            req.add_header("Content-Type", "application/x-www-form-urlencoded")
            with urlopen(req, timeout=15) as response:
                status["sms"] = "SENT" if 200 <= response.status < 300 else f"FAILED: HTTP {response.status}"
        except Exception as exc:
            app.logger.exception("High-risk SMS notification failed")
            status["sms"] = f"FAILED: {type(exc).__name__}"
    return status
...
def analyze_audio(audio: np.ndarray, context: dict[str, Any]) -> dict[str, Any]:
    ...
    classification = build_classification_explanation(model_score, float(np.mean(v2_scores)), float(np.mean(v4_scores)))
    return {
        ...
        "alert_status": {"triggered": False, "email": "PENDING", "sms": "PENDING"},
        "timeline": timeline,
        "context": context,
...
def predict():
    ...
    try:
        ...
        if result["security"]["risk_level"] == "HIGH":
            ...
            result["alert_status"] = send_high_risk_notifications(result)
            storage.append_audit("HIGH_RISK_ALERTS", {"analysis_id": result["analysis_id"], "email": result["alert_status"]["email"], "sms": result["alert_status"]["sms"]})
        else:
            result["alert_status"] = {"triggered": False, "email": "NOT_REQUIRED", "sms": "NOT_REQUIRED", "message": "Notifications are sent only for HIGH risk."}
