...
The prototype now displays the recommended security action, an explainable REAL/FAKE classification reason, and the delivery status of high-risk alerts. When the security risk is HIGH, it automatically attempts to send an email and/or SMS if credentials are configured. It never fabricates a sent alert.

### Email (SMTP)
Set these environment variables before starting Flask:
- `VOICESHIELD_ALERT_EMAIL_TO`
- `VOICESHIELD_SMTP_HOST`
- `VOICESHIELD_SMTP_PORT` (default `587`)
- `VOICESHIELD_SMTP_USER`
- `VOICESHIELD_SMTP_PASSWORD`
- `VOICESHIELD_SMTP_FROM`

Example PowerShell:
```powershell
$env:VOICESHIELD_ALERT_EMAIL_TO="security@example.com"
$env:VOICESHIELD_SMTP_HOST="smtp.example.com"
$env:VOICESHIELD_SMTP_PORT="587"
$env:VOICESHIELD_SMTP_USER="alerts@example.com"
$env:VOICESHIELD_SMTP_PASSWORD="YOUR_APP_PASSWORD"
$env:VOICESHIELD_SMTP_FROM="alerts@example.com"
python app.py
```

### SMS (Twilio)
Set `VOICESHIELD_TWILIO_ACCOUNT_SID`, `VOICESHIELD_TWILIO_AUTH_TOKEN`, `VOICESHIELD_TWILIO_FROM`, and `VOICESHIELD_TWILIO_TO`. The app uses Twilio's HTTPS API and reports `SENT`, `NOT_CONFIGURED`, or `FAILED` on screen.

For a hackathon demo, configure a test email/SMS account rather than embedding credentials in source code.
